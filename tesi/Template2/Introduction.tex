...
The traduction of the model follow the normal steps a geenric translator. There is a first division of the text into lexeme from a Lexer. Then a parsing session and in the end the model is rewritten. 
The main objective of GRIP is to convert multiple full simmetrical models into a single one. The lexer requires a model where there is a model used as template and then multiple modules which uses this template. 
The main transformation usually tries to focus each model in just one and create differents states for the little differences present in the model. 
